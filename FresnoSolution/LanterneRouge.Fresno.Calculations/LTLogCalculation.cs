...
{
    public class LTLogCalculation : BaseCalculation
    {
        #region Fields
        ...
        public LTLogCalculation(IEnumerable<IMeasurementEntity> measurements) : base(measurements)
        { }
        ...
        private double CalculateLT()
        {
            ...
            var x = Loads.Select(l => Math.Log(l)).ToArray();
            var y = Lactates.Select(l => Math.Log(l)).ToArray();
