using LanterneRouge.Fresno.netcore.Calculations;
...
namespace LanterneRouge.Fresno.netcore.Report.PlotModels
{
    public static class StepTests
    {
        ...
        public static PlotModel StepTestPlotModel(IEnumerable<StepTest> stepTests)
        {
            ...
            var stepTestsModel = new PlotModel { Title = $"{testPlotType} of user {string.Join(", ", stepTests.Select(st => st.ParentUser.LastName).Distinct())}", LegendPosition = LegendPosition.LeftTop };
